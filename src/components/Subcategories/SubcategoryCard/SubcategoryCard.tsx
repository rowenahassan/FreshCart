...
export default function SubcategoryCard({subCategory}: {subCategory: SubcategoryType}) {
  return (
    <Link
      className="group bg-white rounded-[16px] border border-gray-100 p-6 shadow-sm hover:shadow-xl hover:border-primary-200 transition-all duration-300 hover:-translate-y-1"
      href={`/products?subcategory=${subCategory._id}`}
    >
      <div className="w-14 h-14 rounded-[12px] bg-primary-50 flex items-center justify-center mb-4 group-hover:bg-primary-100 transition-colors">
        <svg
          data-prefix="fas"
          data-icon="folder-open"
          className="w-7.5 h-6 text-primary-600"
          role="img"
          viewBox="0 0 576 512"
          aria-hidden="true"
        >
