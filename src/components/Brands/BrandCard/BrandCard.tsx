...
export default function BrandCard({brand}: {brand: BrandType}) {
  return (
    <Link
      className="group bg-white rounded-[16px] border border-gray-100 p-4 sm:p-5 shadow-sm hover:shadow-xl hover:border-violet-200 transition-all duration-300 hover:-translate-y-1"
      href={`/products?brand=${brand._id}`}
    >
      <div className="aspect-square rounded-[12px] overflow-hidden bg-gray-50 mb-3 p-4 flex items-center justify-center">
        <Image
          alt={brand.name}
